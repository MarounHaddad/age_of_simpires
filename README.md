...
The mod focuses on building and expanding the city. For that end, the house population capacity was dropped to 2 pop per house. New building maintenance mechanic, as well as health, crime management, and tax collection, force the player to lay the city in a more structured and thoughtful way.

The mod also introduces a small cast system for training the unique units as well as new military structures (moats, barricades, military campls etc.) and concepts such as buildings that cost food, temporary buildings, and structures that cost "unit kills".
...
The sprites for the buildings were generated by LoRA fine tuning a stable diffusion model on the in-game buildings images.
