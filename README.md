...
The mod focuses on building and expanding the city. For that end, the house population capacity was dropped to 2 pop per house. New building maintenance mechanic, as well as health, crime management, and tax collection, force the player to lay out the city in a more structured and thoughtful way.

The mod also introduces a small cast system for training the unique units as well as new military structures (moats, barricades, military camps, etc.) and concepts such as buildings that cost food, temporary buildings, and structures that cost "unit kills".

New buildings were added with graphics for the different architecture sets and a standardized generic destruction animation.

A new menu mechanic extends the villager menu to include the new structures and enables some of the "In Editor" only buildings.

⚠️*The mod was lightly tested and most probably introduces a lot of unintended behavior and even bugs to the base game*
⚠️*Disable `Enhanced Graphics(UHD)` if you have it ON otherwise the New Buildings will not Show*

The sprites for the buildings were generated by fine-tuning a Stable Diffusion model using LoRA on in-game building images.
Model [link](https://huggingface.co/MaroonH89/aoe2_buildings_30k_512_372) on Hugging Face.

Tools:
- Advanced Genie Editor 2
- SLX Studio
- SMX Workshop
...
This unit can collect the taxes from the houses and deposit them in the **Town center**. It collects the taxes at a rate of `3` per trip.

⚠️The `Tax Collector official` has to be within the range of the `Tax Collector` building for him to be able to operate.
